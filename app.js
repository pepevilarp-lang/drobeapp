...
const DROBE_VIDEO_URL=''; // cuando grabes el vídeo del tutorial, pon aquí su URL (YouTube/Vimeo/mp4)
...
function openSettingsSheet(kind){
  ...
  if(kind==='cuenta'){
    ...
    body=`
      ...
      <div class="sh-row" style="margin-top:16px"><span>Versión</span><b>v65</b></div>
...
function vPerfil(m){
  ...
  m.querySelector('#p_tour')?.addEventListener('click',()=>{
    const old=document.getElementById('tutsheet'); if(old)old.remove();
    const w=document.createElement('div'); w.id='tutsheet'; w.className='sheet-ov';
    w.innerHTML=`<div class="sheet"><div class="sheet-bar"></div><div class="sheet-t">Aprende Drobe</div>
      <button class="btn dark" id="tut_tour" style="margin-top:4px">${svg('spark',17)} Tour guiado por la app</button>
      <button class="btn ghost" id="tut_video" style="margin-top:10px">${svg('eye',17)} Ver vídeo (1 min)</button>
      <button class="btn ghost" id="tut_close" style="margin-top:10px">Cerrar</button></div>`;
    document.body.appendChild(w);
    requestAnimationFrame(()=>w.classList.add('show'));
    const close=()=>{w.classList.remove('show');setTimeout(()=>w.remove(),300);};
    w.onclick=e=>{if(e.target===w)close();};
    w.querySelector('#tut_close').onclick=close;
    w.querySelector('#tut_tour').onclick=()=>{ close(); try{localStorage.removeItem('drobe.tour');}catch(e){} setTimeout(startTour,350); };
    w.querySelector('#tut_video').onclick=()=>{
      if(DROBE_VIDEO_URL){ window.open(DROBE_VIDEO_URL,'_blank'); }
      else toast('El vídeo se está grabando — de momento, el tour guiado te lo enseña todo.');
    };
  });
