...
/* `val` puede venir de la IA o de un ticket, es decir, de fuera. Antes se
   ...
   CLAUDE.md. */
function optSel(opts,val){
  const v=val==null?'':String(val);
  return (v&&!opts.includes(v)?`<option selected>${esc(v)}</option>`:'')
...
function vEstilista(m){
  const intents=[['hoy','¿Qué me pongo hoy?'],['dedonde','¿De dónde es?'],['viaje','Preparar viaje'],['tienda','Estoy en tienda'],['comprar','¿Me lo compro?'],['vender','¿Qué vendo?']];
...
/* ═══════════════════════════════════════════
   CLOUD + ARRANQUE
═══════════════════════════════════════════ */
/* ═══ NAVEGACIÓN ATRÁS ROBUSTA ═══
   El gesto atrás y el botón del navegador cierran el overlay superior.
   Y si el binding de un botón "volver" falló por cualquier error previo,
   una red de seguridad delegada lo cierra igualmente: NUNCA una pantalla
   de la que no se pueda salir. */
function topOverlay(){ const ovs=document.querySelectorAll('.ficha,.sheet-ov'); return ovs.length?ovs[ovs.length-1]:null; }
window.addEventListener('popstate',()=>{ const ov=topOverlay(); if(ov){ try{stopChatPoll();}catch(e){} ov.remove(); } });
new MutationObserver(()=>{ const ov=topOverlay(); if(ov&&!ov._hist){ ov._hist=1; try{history.pushState({ov:1},'');}catch(e){} } }).observe(document.body,{childList:true});
document.addEventListener('click',e=>{
  const b=e.target.closest('.backbar button,.ficha-close');
  if(!b)return;
  const ov=b.closest('.ficha,.sheet-ov');
  if(!ov)return;
  setTimeout(()=>{ if(ov.isConnected&&!b.onclick){ try{stopChatPoll();}catch(err){} ov.remove(); } },80);
},true);
