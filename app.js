...
async function loadBgLib(){
  ...
  _bgLib=await import('https://esm.sh/@imgly/background-removal@1.5.5').catch(()=>null);
  return _bgLib;
}
async function studioPhoto(dataUrl){
  const lib=await loadBgLib();
  if(!lib||!lib.removeBackground) throw new Error('No se pudo cargar el motor de recorte.');
  const srcBlob=await (await fetch(dataUrl)).blob();
  const cutBlob=await lib.removeBackground(srcBlob);           // PNG con transparencia
  ...
  if(stBtn&&img){
    const runStudio=async()=>{
      ...
      try{
        ...
        const pv=stage.querySelector('#scanprev img'); if(pv)pv.src=out;
...
/* ═══ ARMARIO PÚBLICO (?u=usuario): vista sin login, sin precios ═══ */
async function tryPublicView(){
  const uname=new URLSearchParams(location.search).get('u');
  if(!uname)return false;
  const app=document.getElementById('app');
  app.innerHTML=`<main id="main" style="padding-top:calc(env(safe-area-inset-top) + 26px)">
    <div class="empty" style="padding-top:80px">${svg('load',26)}<div style="margin-top:12px">Abriendo armario…</div></div></main>`;
  const data=await cloud.getPublicWardrobe(uname).catch(()=>null);
  if(!data){
    app.innerHTML=`<main id="main" style="padding-top:calc(env(safe-area-inset-top) + 26px)">
      <div class="empty" style="padding-top:80px">${svg('lock',26)}<div style="margin-top:12px">Este armario no existe o no es público.</div>
      <a href="${location.pathname}" class="btn dark" style="margin-top:20px;max-width:240px;text-decoration:none">Abrir Drobe</a></div></main>`;
    return true;
  }
  const {profile:sp,garments:gs}=data;
  const visible=gs.filter(g=>g.status!=='venta');
  app.innerHTML=`<main id="main" style="padding-top:calc(env(safe-area-inset-top) + 26px);padding-bottom:40px">
    <div class="pub-head reveal">
      ${avHTML(sp.avatar||(sp.username[0]||'U').toUpperCase())}
      <div>
        <div class="title" style="font-size:28px">El armario de<br>${esc(sp.display_name||sp.username)}</div>
        <div class="sub">@${esc(sp.username)} · ${visible.length} prendas</div>
      </div>
    </div>
    <div class="grid" style="margin-top:20px">${visible.map((g,i)=>`
      <div class="gcard reveal" style="animation-delay:${Math.min(i,10)*0.04}s">
