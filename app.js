  ...
  $('#sc_go').onclick=async function(){
    ...
    if(alts.length){
      html+=`<div class="shead"><h2>Parecidas${fam?' · '+esc(fam.nombre.toLowerCase()):''}</h2></div>
        ...
        ${alts.map(a=>`<a class="alt-card" data-alt="${esc(a.id)}" href="${esc(buscarEnGoogle(`${a.marca} ${a.modelo} ${colorTxt}`.trim()))}" target="_blank" rel="noopener">
      ...
      html+=`<div class="shead"><h2>En marcas que ya usas</h2></div>
        ${marcasSuyas.map(b=>`<a class="alt-card" data-alt="${esc(slug(b))}" href="${esc(buscarEnGoogle(`${b} ${p.cat||''} ${colorTxt}`.trim()))}" target="_blank" rel="noopener">
    ...
    const body={mode:'tienda',sex:u.sex,avgPrice:u.avgPrice,
...
Mira en tienda: "${[brand,p.modelo?.modelo,p.color,p.cat].filter(Boolean).join(' ')||q}"${price?` por ${price}€`:''}${storeName?` en ${storeName}`:''}.`;
    ...
    if(d.sinServicio){
      ...
      ex.innerHTML=(ahorro&&ahorro>0?`<div class="save-banner">${svg('tag',18)} <span>La tienes <b>${ahorro} € más barata</b> en ${esc(mejor.source)}</span></div>`:
        ...
        +d.exacta.slice(0,4).map((o,i)=>`<a class="offer${i===0?' best':''}" href="${esc(o.link)}" target="_blank" rel="noopener">
    ...
    out.querySelectorAll('.alt-card[data-alt]').forEach(card=>{
      ...
      if(lista.length){
        ...
        slot.innerHTML=a?`<span class="alt-pv suele">~${a.precio} €</span><span class="alt-ps">suele costar</span>`:`<span class="alt-ps">Ver precios</span>`;
      }
    });
    if(d.usados&&d.usados.length){
      out.querySelector('#sc_used').innerHTML=`<div class="shead" style="margin-top:18px"><h2>${svg('sync',15)} De segunda mano</h2></div>`
        +d.usados.map(o=>`<a class="offer" href="${esc(o.link)}" target="_blank" rel="noopener"><div class="offer-img">${o.thumbnail?`<img loading="lazy" src="${esc(o.thumbnail)}" alt=""/>`:svg('tag',20)}</div><div class="offer-info"><div class="offer-t">${esc(o.title)}</div><div class="offer-s">${esc(o.source)}</div></div><div class="offer-p">${esc(o.price||'')}</div></a>`).join('');
    }
    ...
    if(aiBox){
      ...
      aiBox.innerHTML=lista.length?`<div class="shead"><h2>Parecidas</h2></div>`+lista.map(a=>`<a class="alt-card" href="${esc(buscarEnGoogle(`${a.marca.marca} ${a.modelo} ${colorTxt}`.trim()))}" target="_blank" rel="noopener">
          ...
          <div class="alt-p"><span class="alt-ps">Ver precios</span></div></a>`).join('')+`<div class="sub" style="font-size:11px;margin-top:6px">Sugerencias de la IA, sin precio en vivo.</div>`:'';
    }
