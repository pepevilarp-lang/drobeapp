...
function styleMatchScore(o,P){
  const t=((o.title||'')+' '+(o.source||'')).toLowerCase();
  let score=50;
  if(P.topBrands.some(b=>t.includes(b.toLowerCase())))score+=18;
  else if([...P.wishBrands,...P.scanBrands,...P.adjBrands,...(P.likedBrands||[])].some(b=>b&&t.includes(String(b).toLowerCase())))score+=12;
  if(P.soldBrands.some(b=>b&&b!=='—'&&t.includes(b.toLowerCase())))score-=15;
  if(P.colors.some(c=>c&&c!=='—'&&t.includes(c.toLowerCase())))score+=9;
  if(P.materials.some(mt=>mt&&t.includes(mt.toLowerCase())))score+=7;
  if(o.price_value&&P.avgPrice){
    const r=o.price_value/P.avgPrice;
    if(r>=0.4&&r<=1.8)score+=9; else if(r<0.25||r>3)score-=18;
  } else if(o.price_value) score+=3;
  if(P.plainRatio>0.7&&/estampad|graphic|print|dibujo|anime|calavera|oversize logo|big logo/i.test(t))score-=25;
  if(/zalando|corte ingl|asos\b|about you|scalpers|massimo|zara\b|mango\b|nike|adidas|farfetch|end\.|mr ?porter|footdistrict|deporvillage|tradeinn|decathlon/i.test(t))score+=8;
  return Math.max(0,Math.min(100,Math.round(score)));
}

/* ═══ CURACIÓN: puntúa, filtra por umbral y diversifica como un estilista ═══ */
function curateOffers(items,{threshold=90,maxPerBrand=3}={}){
  const P=fashionProfile();
  const scored=items.map(o=>({o,s:styleMatchScore(o,P)})).filter(x=>x.s>=threshold)
    .sort((a,b)=>b.s-a.s);
...
function openSettingsSheet(kind){
  ...
  if(kind==='cuenta'){
    ...
    body=`
      ...
      <div class="sh-row" style="margin-top:16px"><span>Versión</span><b>v60</b></div>
...
async function appendUsedSection(container,params){
  ...
  const items=curateOffers(used?[...(used.exact||[]),...(used.alternatives||[])]:[],{threshold:60});
...
async function searchOffersExtensive({query,brand,productType,maxPrice,ownedBrands,sex,channel,avgPrice}){
  ...
  const curate=d=>({exact:d.exact||[],alternatives:(channel==='used')?(d.alternatives||[]):curateOffers(d.alternatives||[],{threshold:85})});
...
async function openParaTi(){
  ...
  const curated=curateOffers(items,{threshold:90});
  if(!curated.length){
...
Devuelve SOLO JSON: {"resumen":"1 frase sobre su armario y su estilo","faltas":[{"prenda":"tipo concreto","motivo":"por qué le cundiría dado SU estilo","busqueda":"marca(preferiblemente suya) + tipo + color + ${u.sex||''}"}]}. Máximo 4 faltas.`;
  ...
  callAI(sys,usr).then(async r=>{
    ...
    out.querySelectorAll('.gap').forEach(g=>g.querySelector('.gap-btn').onclick=async function(){
      ...
      const nItems=curateOffers(nuevo?[...(nuevo.exact||[]),...(nuevo.alternatives||[])]:[],{threshold:90});
      const uItems=curateOffers(used?[...(used.exact||[]),...(used.alternatives||[])]:[],{threshold:60});
      let html='';
...
async function openFriendGarment(g){
  ...
  wrap.querySelector('#fg_find').onclick=async function(){
    this.disabled=true; this.innerHTML=`${svg('load',17)} Buscando…`; this.querySelector('svg').classList.add('spin');
    const u=userContext();
    const out=wrap.querySelector('#fg_out');
    const r=await searchOffersExtensive({query:`${g.brand||''} ${g.name||''}`.trim(),brand:g.brand,productType:[g.cat,g.color].filter(Boolean).join(' '),ownedBrands:u.topBrands,sex:u.sex,avgPrice:u.avgPrice,channel:'new'});
    if(!wrap.isConnected)return;
    const nItems=r?[...(r.exact||[]),...(r.alternatives||[])]:[];
    this.style.display='none';
    out.innerHTML=nItems.length
      ?`<div class="shead" style="margin-top:16px"><h2>${svg('tag',15)} Disponible nueva</h2></div>`+nItems.slice(0,4).map(offerRowHTML).join('')
      :`<div class="sub" style="margin-top:14px">No la encuentro nueva ahora mismo — puede estar descatalogada. Aquí tienes toda su ficha, y pruebo en segunda mano:</div>`;
    appendUsedSection(out,{query:`${g.brand||''} ${g.name||''}`.trim(),brand:g.brand,productType:g.cat,ownedBrands:u.topBrands,sex:u.sex,avgPrice:u.avgPrice});
  };
}
