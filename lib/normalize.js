...
[
  ...
  /* Vaqueros. */
  'Nudie Jeans','Edwin','Orslow','Wrangler','Lee','Replay','G-Star','Scotch & Soda',
...
function marcaParecida(k) {
  const max = k.length <= 4 ? 0 : k.length <= 6 ? 1 : 2;
  if (!max) return '';
...
export function canonMarca(raw) {
  ...
  // Quitar coletillas por la derecha: "autry medalist" → "autry".
  const pal = k.split(' ').filter(Boolean);
  while (pal.length > 1 && COLETILLAS.has(pal[pal.length - 1])) pal.pop();
  while (pal.length > 1 && COLETILLAS.has(pal[0])) pal.shift();
  const limpio = pal.join(' ');
  if (limpio !== k) {
    if (ALIAS_MARCA[limpio]) return ALIAS_MARCA[limpio];
    if (MARCAS[limpio]) return MARCAS[limpio];
  }
  ...
  for (let n = pal.length; n >= 1; n--) {
    ...
    if (MARCAS[pre]) return MARCAS[pre];
    if (ALIAS_MARCA[pre]) return ALIAS_MARCA[pre];
  }

  const juntas = limpio.replace(/ /g, '');
  if (MARCAS_JUNTAS[juntas]) return MARCAS_JUNTAS[juntas];
