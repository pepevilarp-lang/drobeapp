...
// Banderas de "ya preparado" para la cuenta vigente. Se resetean en setUid().
...
/* ═══ QUIÉN ESTÁ DENTRO ═══
   Toda escritura resolvía el usuario en el momento de ENVIAR, no en el momento
   de encolar. Como las escrituras se disparan sin await (addGarment, editar
   ficha, tickets…), una que arrancaba con la cuenta A y resolvía getUser()
   después de un cambio de cuenta se grababa con el user_id de B: la prenda de
   uno acababa dentro del armario del otro.

   Ahora el uid vigente vive aquí, se actualiza en cada cambio de sesión, y
   cada escritura lo captura de forma síncrona al entrar y comprueba que sigue
   siendo el mismo antes de tocar la base de datos. */
let _uid = null;
function setUid(u) {
  const next = u || null;
  if (next !== _uid) {
    // cambio de cuenta: los flags de "ya preparado" pertenecían al anterior
...
/* Devuelve el usuario solo si sigue siendo aquel para el que se encoló la
   operación. Si la cuenta cambió por el camino, aborta en vez de escribir. */
async function authFor(queuedFor) {
  const c = await client(); if (!c) return { c:null, user:null, reason:'no_client' };
  const { data: { user } } = await c.auth.getUser();
  if (!user) return { c, user:null, reason:'no_session' };
  setUid(user.id);
  if (queuedFor && queuedFor !== user.id) {
    logError('cloud.cuentaCambiada', 'operación encolada con otra cuenta; descartada', { extra:{ queuedFor, ahora:user.id } });
    return { c, user:null, reason:'account_changed' };
  }
  return { c, user, reason:null };
}
...
export async function onAuth(cb) {
  ...
  c.auth.onAuthStateChange((_e, s) => { setUid(s?.user?.id); cb(s); });
}
...
export async function signOut() {
  _profileEnsured = false; _socialEnsured = false; _uid = null;
...
export function fromRow(r) {
  ...
  return {
    ...
    // `photos` estaba declarada dos veces en este mismo objeto y la segunda
    // (`photos: []`) ganaba: las fotos extra se subían a Supabase y se
    // borraban en el siguiente pull. Una sola vez, y con las de verdad.
...
export async function pullGarments() {
  ...
  if (error) { logError('cloud.pull', error); return null; }
  return data;
}
export async function pushGarment(g, forceInsert = false) {
  const queuedFor = _uid;                       // síncrono: la cuenta de AHORA
  const { c, user, reason } = await authFor(queuedFor);
  ...
  if (error && /column|schema/i.test(error.message || '')) {
    ...
    if (!retry.error) { logError('cloud.pushGarment.schema', 'guardado sin las columnas nuevas — falta ejecutar supabase/schema.sql'); return { ok:true }; }
    error = retry.error;
  }
  if (error) {
...
export async function deleteGarmentCloud(id) {
  ...
  const queuedFor = _uid;
  const { c, user, reason } = await authFor(queuedFor);
  if (!user) return { ok:false, reason };
  try {
    // .eq('user_id') además de RLS: filtrar solo por id dejaba el borrado a
    // merced de que las políticas estuvieran bien puestas, y los id los genera
    // el cliente.
    ...
    if (!error) {
      ...
      // Antes se comprobaba si la fila seguía siendo VISIBLE, y una fila ajena
      // o bloqueada por RLS no lo es: el borrado fallido se registraba como
      // éxito y se tiraba el tombstone. Ahora se pregunta por MI fila.
      const { data: still } = await c.from('garments')
        .select('id').eq('id', id).eq('user_id', user.id).maybeSingle();
      return { ok: !still };                                   // ya no es mía = nada que borrar
    }
    if (/column|schema/i.test(error.message || '')) {          // SQL de upgrade sin ejecutar: borrado duro verificado
      await c.from('garments').delete().eq('id', id).eq('user_id', user.id);
      const { data: still } = await c.from('garments')
        .select('id').eq('id', id).eq('user_id', user.id).maybeSingle();
      return { ok: !still };
    }
    logError('cloud.deleteGarment', error);
    return { ok:false };
  } catch(e) { logError('cloud.deleteGarment', e); return { ok:false }; }
}
...
export async function ensureProfile() {
  ...
  if (error) { logError('cloud.ensureProfile', error); return false; }
...
export async function uploadTicketImage(ticketId, dataUrl) {
  ...
  try {
    ...
    if (error) { logError('cloud.uploadTicket', error); return null; }
    const { data } = c.storage.from('tickets').getPublicUrl(path);
    return { url: data.publicUrl, path };
  } catch(e) { logError('cloud.uploadTicket', e); return null; }
}
export async function saveTicketCloud(t) {
  ...
  if (error) { logError('cloud.saveTicket', error); return { ok:false, reason:[error.message,error.code,error.details].filter(Boolean).join(' | ') }; }
  return { ok:true };
}
...
export async function pullTickets() {
  const c = await client(); if (!c) return null;
  const { data: { user } } = await c.auth.getUser(); if (!user) return null;
  const { data, error } = await c.from('tickets').select('*').eq('user_id', user.id).order('created_at', { ascending: false });
  if (error) { logError('cloud.pullTickets', error); return null; }
  return data;
}
...
export async function getFriendWardrobe(friendId) {
  const c = await client(); if (!c) return null;
  // sin este filtro, una prenda que el dueño había borrado seguía viéndose
  // para siempre en el armario de sus amigos
  const { data, error } = await c.from('garments').select('*').eq('user_id', friendId).is('deleted_at', null);
  if (error) { logError('cloud.friendWardrobe', error); return null; }
...
export async function uploadAvatar(dataUrl) {
  ...
  try {
    ...
    if (error) { logError('cloud.uploadAvatar', error); return null; }
    const { data } = c.storage.from('tickets').getPublicUrl(path);
    ...
    return url;
  } catch(e) { logError('cloud.uploadAvatar', e); return null; }
}
...
export async function saveWishlistCloud(w){
  ...
  if(error){ logError('cloud.wishlist', error); return {ok:false,reason:error.message}; }
  return {ok:true};
}
...
export async function pullWishlist(){
  const c=await client(); if(!c) return null;
  const { data: { user } } = await c.auth.getUser(); if (!user) return null;
  const { data,error }=await c.from('wishlist').select('*').eq('user_id', user.id).order('created_at',{ascending:false});
  if(error){ logError('cloud.pullWishlist', error); return null; }
  return data;
}
...
export async function pullProfile() {
  ...
  if (error) { logError('cloud.pullProfile', error); return null; }
  return data;
}
...
export async function saveMaletaCloud(mal) {
  ...
  if (error) { logError('cloud.saveMaleta', error); return { ok:false, reason:error.message }; }
  return { ok:true };
}
...
export async function pullMaletas() {
  const c = await client(); if (!c) return null;
  const { data: { user } } = await c.auth.getUser(); if (!user) return null;
  const { data, error } = await c.from('maletas').select('*').eq('user_id', user.id).order('created_at', { ascending: false });
  if (error) { logError('cloud.pullMaletas', error); return null; }
  return data;
}
...
export async function getPublicWardrobe(username) {
  ...
  const { data: gs } = await c.from('garments').select('*').eq('user_id', sp.id).is('deleted_at', null).order('created_at', { ascending:false });
