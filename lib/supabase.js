...
export function fromRow(r) {
  ...
  return {
    ...
    worn: r.worn || 0, lastWorn: r.last_worn, lastWornAt, fitFeedback: r.fit_feedback || null, fitFeedback: r.fit_feedback || null,
...
export async function searchUsers(q) {
  const c = await client(); if (!c) return [];
  const { data: { user } } = await c.auth.getUser();
  const term = String(q||'').toLowerCase().replace(/[^a-z0-9_.]/g,'');
  if (term.length < 2) return [];
  const { data } = await c.from('social_profiles').select('id,username,display_name,avatar').ilike('username', term+'%').limit(10);
