...
import { slug, canonMarca, canonColor, canonMaterial, canonTalla, canonTienda,
         canonPatron, grupoDe, GRUPOS, esNeutro, COLORES } from './normalize.js';
...
export function buildTasteProfile(store, { context = 'calle', now = new Date() } = {}) {
  ...
  /* ── techo de precio aprendido ──
     Si rechaza cosas por caras, el techo baja hacia lo que sí paga. El escáner
     guarda TAMBIÉN el precio y la categoría de lo que rechazó; antes se
     ignoraban y solo se contaba "rechazó algo". */
  ...
  // techo concreto por categoría: el precio más BAJO que ya ha rechazado por caro
  const techoPorCat = {};
  tooExpensive.forEach(s => {
    const k = grupoDe(s.cat) || grupoDe(s.query) || '';
    const p = Number(s.price) || 0;
    if (!k || !p) return;
    techoPorCat[k] = techoPorCat[k] ? Math.min(techoPorCat[k], p) : p;
  });
  ...
  /* El patrón lo dice ahora la IA (y el usuario lo corrige). Antes solo se
     adivinaba con un regex sobre el nombre y sobre unas etiquetas que no había
     forma de poner desde la app, así que la señal era casi siempre ruido. */
  const conPatron = active.filter(g => canonPatron(g.pattern));
  const printed = conPatron.length
    ? conPatron.filter(g => canonPatron(g.pattern) !== 'Liso').length
    : active.filter(g => PRINT_RX.test((g.name || '') + ' ' + (g.tags || []).join(' '))).length;
  const base = conPatron.length || n;
  const plainRatio = base ? 1 - printed / base : 0.7;
  ...
  /* ── cómo talla cada marca, según el propio usuario ──
     El usuario podía decir en la ficha "esta marca talla pequeño" y ese dato no
     llegaba al motor: se usaba solo en la demo para marcas. Ahora se agrega por
     marca y se convierte en un aviso al recomendar. */
  const tallajeVotos = {};
  active.forEach(g => {
    const fb = g.fitFeedback, m = canonMarca(g.brand);
    if (!fb || !m) return;
    (tallajeVotos[m] = tallajeVotos[m] || {})[fb] = (tallajeVotos[m][fb] || 0) + 1;
  });
  const tallaje = {}; Object.entries(tallajeVotos).forEach(([m, v]) => { tallaje[m] = pickTop(v); });
  ...
  return {
    ...
    priceBands, globalBand, avgPrice, segment, priceCeilingFactor, techoPorCat,
    ...
    sizeByCat, sizeByBrand, tallaje, trustedStores, rejections,
...
export function scoreOffer(offer, P, hints = {}) {
  ...
  /* ── ya rechazó algo así por caro, y esto cuesta más ── */
  if (o.cat && o.price && P.techoPorCat && P.techoPorCat[o.cat] && o.price >= P.techoPorCat[o.cat]) {
    cut(20, `Ya dejaste algo así por ${Math.round(P.techoPorCat[o.cat])}€`);
  }
  ...
  const marcaO = canonMarca(o.brand);
  const wantSize = marcaO ? P.sizeByBrand[marcaO] : (o.cat ? P.sizeByCat[o.cat] : '');
  if (wantSize) reasons.push(`Tu talla en esto suele ser ${wantSize}`);
  // lo que el propio usuario dijo del tallaje de esta marca
  const fb = marcaO ? (P.tallaje || {})[marcaO] : '';
  if (fb && /peque/i.test(fb)) warnings.push('Dijiste que esta marca talla pequeño');
  else if (fb && /grande/i.test(fb)) warnings.push('Dijiste que esta marca talla grande');
