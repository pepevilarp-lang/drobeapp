...
const NEUTRALS = ['blanco', 'negro', 'gris', 'crudo', 'marino', 'azul marino', 'beige', 'marrón', 'marron', 'camel', 'topo', 'antracita'];

// color en el título del producto -> color canónico del armario
const COLOR_WORDS = {
  'blanco': 'Blanco', 'white': 'Blanco', 'óptico': 'Blanco',
  'negro': 'Negro', 'black': 'Negro',
  'gris': 'Gris', 'grey': 'Gris', 'gray': 'Gris', 'antracita': 'Gris', 'marengo': 'Gris',
  'marino': 'Marino', 'navy': 'Marino',
  'azul': 'Azul', 'blue': 'Azul', 'celeste': 'Azul', 'denim': 'Azul',
  'verde': 'Verde', 'green': 'Verde',
  'kaki': 'Kaki/Oliva', 'caqui': 'Kaki/Oliva', 'khaki': 'Kaki/Oliva', 'oliva': 'Kaki/Oliva', 'militar': 'Kaki/Oliva',
  'marrón': 'Marrón', 'marron': 'Marrón', 'brown': 'Marrón', 'chocolate': 'Marrón', 'camel': 'Marrón', 'tostado': 'Marrón',
  'beige': 'Beige', 'arena': 'Beige', 'piedra': 'Beige', 'taupe': 'Beige',
  'crudo': 'Crudo', 'ecru': 'Crudo', 'hueso': 'Crudo', 'off white': 'Crudo', 'natural': 'Crudo',
  'rojo': 'Rojo', 'red': 'Rojo', 'burdeos': 'Rojo', 'granate': 'Rojo', 'vino': 'Rojo',
  'amarillo': 'Amarillo', 'mostaza': 'Amarillo',
  'naranja': 'Naranja', 'teja': 'Naranja',
  'rosa': 'Rosa', 'pink': 'Rosa', 'salmón': 'Rosa',
  'morado': 'Morado', 'lila': 'Morado', 'violeta': 'Morado'
};

// palabra en el título -> grupo de categoría del armario
const CAT_WORDS = {
  'Camisetas': ['camiseta', 't-shirt', 'tshirt', 'tee ', 'polo', 'manga corta', 'manga larga'],
  'Camisas': ['camisa', 'shirt', 'overshirt', 'sobrecamisa'],
  'Jerséis/Sudaderas': ['jersey', 'sudadera', 'hoodie', 'sweatshirt', 'sweater', 'cardigan', 'punto', 'knit', 'crewneck'],
  'Pantalones': ['pantalón', 'pantalon', 'vaquero', 'jean', 'chino', 'cargo', 'jogger', 'trouser', 'pants', 'short', 'bermuda'],
  'Abrigos/Chaquetas': ['abrigo', 'chaqueta', 'cazadora', 'parka', 'plumífero', 'plumifero', 'bomber', 'blazer', 'trench', 'anorak', 'jacket', 'coat', 'chaleco', 'gabardina'],
  'Calzado': ['sneaker', 'zapatilla', 'bota', 'botín', 'botin', 'zapato', 'mocasín', 'mocasin', 'sandalia', 'deportiva', 'shoe', 'boot'],
  'Accesorios': ['gorra', 'bufanda', 'cinturón', 'cinturon', 'mochila', 'bolso', 'gafas', 'guantes', 'calcetines', 'cartera', 'gorro']
};
...
export function buildTasteProfile(store, { context = 'calle', now = new Date() } = {}) {
  ...
  const brandRaw = {};
  const bump = (b, v) => { if (b && b !== '—' && b !== '-') brandRaw[b] = (brandRaw[b] || 0) + v; };
  ...
  active.forEach(g => {
    ...
    (g.colors && g.colors.length ? g.colors : [g.color]).filter(Boolean).forEach(c => {
      colorWeighted[c] = (colorWeighted[c] || 0) + w; colorTotal += w;
    });
  });
  Object.keys(colorWeighted).forEach(c => { colorWeighted[c] /= (colorTotal || 1); });

  const neutralRatio = n ? active.filter(g => NEUTRALS.includes(norm(g.color))).length / n : 0.5;
  ...
  active.forEach(g => {
    ...
    const c = g.color; if (c) satColor[k + '|' + c] = (satColor[k + '|' + c] || 0) + 1;
  });

  /* ── huecos: lo contrario de la saturación ── */
  const gaps = Object.keys(CAT_WORDS).filter(k => (saturation[k] || 0) === 0);
  ...
  active.forEach(g => {
    if (!g.size) return;
    const k = g.catGroup || 'Otros';
    (sizeVotes[k] = sizeVotes[k] || {})[g.size] = (sizeVotes[k][g.size] || 0) + 1;
    if (g.brand) (sizeByBrandVotes[g.brand] = sizeByBrandVotes[g.brand] || {})[g.size] = (sizeByBrandVotes[g.brand][g.size] || 0) + 1;
  });
  ...
  const storeShare = share(active, 'store');
  ...
  return {
    ...
    materials: share(active, 'material'),
    topMaterials: Object.entries(share(active, 'material')).sort((a, b) => b[1] - a[1]).slice(0, 4).map(([m]) => m),
...
export function readOffer(offer, P) {
  ...
  candidates.forEach(b => {
    if (brandInText(b, t) && norm(b).length > brandLen) { brand = b; brandLen = norm(b).length; }
  });

  let cat = '';
  for (const [group, words] of Object.entries(CAT_WORDS)) {
    if (words.some(w => t.includes(w))) { cat = group; break; }
  }

  let color = '';
  for (const [word, canon] of Object.entries(COLOR_WORDS)) {
    if (new RegExp('(^| )' + word + '($| )').test(t)) { color = canon; break; }
  }
...
export function scoreOffer(offer, P, hints = {}) {
  ...
  const aff = o.brand ? (P.brandAffinity[o.brand] || 0) : 0;
  ...
  if (o.color) {
    ...
    else if (P.neutralRatio > 0.7 && !NEUTRALS.includes(norm(o.color))) cut(12, 'Color fuera de tu paleta');
  }
...
export const _internals = { norm, monthsAgo, currentSeason, brandInText, quantile, CAT_WORDS, COLOR_WORDS };
